      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-3">
          ...
          <motion.button
            ...
            type="button"
            whileTap={{ scale: 0.9 }}
            whileHover={{ scale: 1.05, rotate: theme === 'galaxy' ? -6 : 6 }}
            transition={{ type: 'spring', stiffness: 260, damping: 18 }}
          >
            <motion.div
              className="relative flex items-center justify-center"
              animate={{ rotate: theme === 'galaxy' ? 0 : 180, scale: theme === 'galaxy' ? 1 : 1.08 }}
              transition={{ type: 'spring', stiffness: 200, damping: 16 }}
            >
              <span className="absolute h-8 w-8 rounded-full bg-white/10 blur-md" aria-hidden />
              {theme === 'galaxy' ? (
